...
const Detail = (props) => {
    ...
    useEffect(() => {
        ...
        return () => {
            // 기존 코드를 치우는거 여기에 많이 작성함
            // clean up function 이라고 함
            clearTimeout(a)
        }
        // useEffect 동작 전에 실행 되는 return () => {}
        // (참고) clean up function은 mount시 실행안됨 unmount시 실행됨
        
    }, [count])
    //useEffect dependency
    // dependency 없을때 : mount, update시 실행됨 (재랜더링마다)
    // dependency [count] : mount 될때 한번, count라는 state가 변할때만 실행됨
    // dependency [] : mount에만 실행됨
